...
return(
    <div className='container max-w-full'>
      ...
      <div className="px-3">
        ...
        {!copyaction ?
        <div className="text-blue-950 text-sm font-medium tracking-wide pr-2 pt-2"> <button className="text-blue-950 text-sm font-medium tracking-wide pr-2" onClick={() => {navigator.clipboard.writeText(coupondeets?.coupon_code); setCopyaction(true)}}> COPY </button> </div>
        :
        ...
        <Button className="bg-white w-60 my-3 h-12 shadow-2xl rounded-xl" onClick={()=>{
          makeApiCallWithAuth('trackEvent', {event: 4, offer: sessionStorage.getItem('id'), mop: 1})
            ...
            navigate('/exitpwa?launch='+coupondeets?.offer_url)}}><span className="text-[#021555] text-lg font-medium -my-2">REDEEM NOW</span>
        </Button>
        :
        <Button className="bg-white w-60 my-3 h-12 shadow-2xl rounded-xl" onClick={()=>{
          makeApiCallWithAuth('trackEvent', {event: 4, offer: sessionStorage.getItem('id'), mop: 1})
            ...
            .catch((e) => console.log("err", e));
            window.open(atob(coupondeets?.offer_url).split('?type=' || '&type=')[0].slice(7), '_blank')?.focus();}}><span className="text-[#021555] text-lg font-medium -my-2">REDEEM NOW</span>
        </Button>
        }
        </div>
