    ...
    <div className='container overflow-y-auto h-screen'>
        ...
        <div className={`absolute w-screen -mt-10 h-72`} style={{ backgroundColor: `${orderDetails?.down_color}`}}>
          <div className={`relative w-screen -mt-10 h-44 rounded-br-[30px]`} style={{ backgroundColor: `${orderDetails?.up_color}`}}></div>
            ...
            <div className="absolute  left-1/2 -translate-x-1/2 bottom-5">
              <img src={orderDetails?.product_pic} className="w-48 h-48" alt="" />
            </div>
          </div>
      ...
      <div className="px-3 pt-1 rounded-t-2xl bg-[#F9F9F9]" style={{ height: `calc(100vh - 450px)` }}>
        {productDeliverable &&
          ...
          >
            <Button disabled={true} onClick={()=>{setOpenSelect(true)}} className='bg-white border-none shadow-none w-fit p-0'><img src={backdark} className='rotate-180 w-3'></img></Button>
          </CustomCard>
        }
        ...
        <div className="py-20"></div>
         {(!productDeliverable && orderDetails?.offer_url)&&
                ...
                <Button 
                onClick={()=>{
                        makeApiCallWithAuth('trackEvent', {event: 4, offer: orderDetails?.id, mop: 11})
                          .then((response) => {
